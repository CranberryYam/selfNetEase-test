...
//上面两个properties就像是两个容器，分别用来装两种类型的object，与subViews完全没关系
@end
...
- (void)addImageView
{
    ...
    //imgView.image = [UIImage imageNamed:@"view1.jpg"];//@2通过改变这里的图片，可以实现自定义TabBar的外形；
...
- (void)layoutSubviews
{
    UIImageView *imgView = self.subviews[0];//这样其实是在传送地址
...
- (void)btnClick:(SXBarButton *)btn
{
    ...
    // 设置按钮显示状态 并切换选中按钮
    _selButton.enabled = YES;//实质是设置前一个Button为灰的，因为_selButton是用来保存前一个Button的
    _selButton = btn;
    btn.enabled = NO;//YES的时候是灰的，NO的时候是红的
}
